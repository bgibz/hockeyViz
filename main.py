...
def plot_wins_data(data, all_games, season, gd):
    my_dict = {
        "Team": data.team_name,
    ...
    filepath = Path.cwd() / "Graphs" / season / gd / file
    sns.barplot(data=df).set_title(data.team_name)
...
def crunch_season_data(season, num_games, num_days):
    ...
    name = season + ".png"
    filepath = Path.cwd() / "Graphs" / season / games_in_days / name
    plt.tight_layout()
    plt.savefig(filepath)
    plt.clf()
    # Save a csv
...
crunch_season_data(season, num_games, num_days)
